...
)
{
  ...
  LIBBLU_DTS_DEBUG_CORE(
    "  Deficit sample count (SHORT): %" PRIu8 " samples (0x%" PRIX8 ").\n",
  ...
  if (param.crcPresent && !warningFlags->presenceOfDeprecatedCrc) {
    LIBBLU_WARNING(
      "Presence of deprecated DCA CRC field.\n"
    );
    warningFlags->presenceOfDeprecatedCrc = true;
  }
  ...
  LIBBLU_DTS_DEBUG_CORE(
    "  Audio channel arrangement code (AMODE): 0x%" PRIX8 ".\n",
  ...
  LIBBLU_DTS_DEBUG_CORE(
    "  Core audio sampling frequency code (SFREQ): 0x%" PRIX8 ".\n",
    param.audioFreqCode
  );
  if (param.audioFreq != 0)
    ...
    LIBBLU_DTS_ERROR_RETURN(
      "Reserved value in use (SFREQ == 0x%" PRIX8 ").\n",
      param.audioFreqCode
    );
  }
  ...
  LIBBLU_DTS_DEBUG_CORE(
    "  Target transmission bitrate code (RATE): 0x%" PRIX8 ".\n",
    param.bitRateCode
  );
  switch (param.bitrate) {
    case 0: /* Reserved value */
      LIBBLU_DTS_DEBUG_CORE(
        "   => Bitrate: Reserved value (0x%" PRIX8 ").\n",
        param.bitRateCode
      );
      LIBBLU_DTS_ERROR_RETURN(
        "Reserved value in use (RATE == 0x%" PRIX8 ").\n",
        param.bitRateCode
      );
  ...
  if (param.hdcd && !warningFlags->usageOfHdcdEncoding) {
    LIBBLU_WARNING(
      "Signaled usage of HDCD mastering mode, "
      "normaly reserved for DTS-CDs.\n"
    );
    warningFlags->usageOfHdcdEncoding = true;
  }
  ...
  if (param.extAudio) {
    ...
    if (!isValidDtsExtendedAudioCodingCode(param.extAudioId))
      LIBBLU_DTS_ERROR_RETURN(
        "Reserved value in use (EXT_AUDIO_ID == 0x%" PRIX8 ").\n",
        param.extAudioId
      );
  }
  ...
  if (param.lfePresent == 0x3)
    LIBBLU_DTS_ERROR_RETURN(
      "Invalid value in use (LFE == 0x%" PRIX8 ").\n",
      param.lfePresent
    );
  ...
  LIBBLU_DTS_DEBUG_CORE(
    "  Encoder sofware syntax revision (VERNUM): %" PRIu8 " (0x%" PRIx8 ").\n",
    param.syntaxCode,
    param.syntaxCode
  );
  ...
  LIBBLU_DTS_DEBUG_CORE(
    "  Copyright history information code (CHIST): 0x%" PRIX8 ".\n",
    param.copyHistory
  );
  ...
  LIBBLU_DTS_DEBUG_CORE(
    "  Source PCM resolution code (PCMR): 0x%" PRIX8 ".\n",
  ...
  if (param.syntaxCode == 0x6 || param.syntaxCode == 0x7) {
    LIBBLU_DTS_DEBUG_CORE(
      "  Dialog normalization code (DIALNORM/DNG): 0x%" PRIX8 ".\n",
      param.dialNormCode
    );
    LIBBLU_DTS_DEBUG_CORE(
...
{
  return
    first.terminationFrame == second.terminationFrame
    && first.samplesPerBlock == second.samplesPerBlock
    && first.blocksPerChannel == second.blocksPerChannel
    && first.crcPresent == second.crcPresent
    && first.audioChannelArrangement == second.audioChannelArrangement
    && first.audioFreqCode == second.audioFreqCode
    && first.sourcePcmResCode == second.sourcePcmResCode
    && first.bitRateCode == second.bitRateCode
    && first.auxData == second.auxData
    && first.hdcd == second.hdcd
    && first.extAudio == second.extAudio
    && first.lfePresent == second.lfePresent
    && first.extAudioId == second.extAudioId
    && first.syntaxCode == second.syntaxCode
  ;
}
...
)
{
  ...
  if (checkDcaCoreSSFrameHeaderCompliance(frame.header, warningFlags) < 0)
    return -1;
...
{
  return
    first.payloadSize == second.payloadSize
    && constantDcaCoreSSFrameHeader(first.header, second.header)
  ;
}
...
{
  if (checkDcaCoreSSCompliance(cur, warningFlags) < 0)
    return -1;

  return checkDcaCoreSSFrameHeaderChangeCompliance(
...
)
{
  ...
  LIBBLU_DTS_DEBUG_EXTSS(
    "   Frame duration code (nuExSSFrameDurationCode): 0x%" PRIX8 ".\n",
  ...
  for (nAuPr = 0; nAuPr < param.nbAudioPresentations; nAuPr++) {
    ...
    if (!param.activeExtSSMask[nAuPr])
      ...
      for (nSS = 0; nSS < nExtSSIndex + 1; nSS++) {
  ...
  if (param.mixMetadataEnabled) {
    if (!warningFlags->nonCompleteAudioPresentationAssetType) {
      LIBBLU_INFO(
        LIBBLU_DTS_PREFIX
        "Presence of Mixing Metadata in Extension Substream.\n"
      );
      warningFlags->nonCompleteAudioPresentationAssetType = true;
    }

    LIBBLU_DTS_DEBUG_EXTSS("   Mixing Metadata:\n");

    if (checkDcaExtSSHeaderMixMetadataCompliance(param.mixMetadata) < 0)
      return -1;
  }
...
)
{
  ...
  if (param.assetTypeDescriptorPresent) {
    LIBBLU_DTS_DEBUG_EXTSS(
      "     Asset type descriptor (nuAssetTypeDescriptor): 0x%" PRIX8 ".\n",
  ...
  if (!param.directSpeakersFeed) {
    if (
      ...
      if (!warningFlags->nonDirectSpeakersFeedTolerance) {
        LIBBLU_WARNING(
        ...
        warningFlags->nonDirectSpeakersFeedTolerance = true;
      }
    }
    else if (
      param.nbChannels == 2
  ...
  if (param.directSpeakersFeed) {
    ...
    if (2 < param.nbChannels) {
      ...
      if (
        param.embeddedStereoDownmix
        && !warningFlags->presenceOfStereoDownmix
      ) {
        LIBBLU_INFO(
          "Presence of an optional embedded Stereo Down-mix "
          "in Extension Substream.\n"
        );
        warningFlags->presenceOfStereoDownmix = true;
      }
    }
    else
    ...
    if (param.speakersMaskEnabled) {
      nbChannelsFromMask = dcaExtChMaskToNbCh(param.speakersMask);
      ...
      if (nbChannelsFromMask != param.nbChannels)
        LIBBLU_DTS_ERROR_RETURN(
    ...
    LIBBLU_DTS_DEBUG_EXTSS("      Representation type code: 0x%" PRIX8 ".\n");
    LIBBLU_DTS_DEBUG_EXTSS(
      "       -> %s.\n",
...
)
{
  ...
  if (param.drcEnabled) {
    ...
    if (staticFields.embeddedStereoDownmix) {
      LIBBLU_DTS_DEBUG_EXTSS(
...
)
{
    ...
    case DCA_EXT_SS_CODING_MODE_DTS_HD_LOW_BITRATE:
      ...
      if (
        ...
        if (param.codingComponents.extSSXll.syncWordPresent) {
          ...
          LIBBLU_DTS_DEBUG_EXTSS(
            "        DTS-HD XLL Stream ID (nuDTSHDStreamID): 0x%" PRIX8 ".\n",
  ...
  if (param.mixMetadataFieldsPresent) {
    ...
    if (param.mixMetadata.oneTrackToOneChannelMix) {
      ...
      for (ns = 0; ns < mixMetadata.nbMixOutputConfigs; ns++) {
        LIBBLU_DTS_DEBUG_EXTSS("       - Config %u: ", ns);
        if (param.mixMetadata.perChannelMainAudioScaleCode) {
          for (nCh = 0; nCh < mixMetadata.nbMixOutputChannels[ns]; nCh++) {
            if (0 < nCh)
              LIBBLU_DTS_DEBUG_EXTSS_NH(", ");
            LIBBLU_DTS_DEBUG_EXTSS_NH(
...
)
{
  ...
  if (param.staticFieldsPresent) {
    ...
    if (
      checkDcaAudioAssetDescriptorStaticFieldsCompliance(
        param.staticFields,
        isSecondaryStream,
        warningFlags
      ) < 0
  ...
  if (
    checkDcaAudioAssetDescriptorDynamicMetadataCompliance(
      param.dynMetadata,
      param.staticFields
    ) < 0
  )
  ...
  if (
    checkDcaAudioAssetDescriptorDecoderNavDataCompliance(
      param.decNavData,
      mixMetadata,
      isSecondaryStream
    ) < 0
  ...
  if (
    ...
    for (
      i = 0;
      i < param.reservedFieldLength + (param.paddingBits + 7) / 8;
      i++
    ) {
...
)
{
  ...
  LIBBLU_DTS_DEBUG_EXTSS(
    "  User defined bits (UserDefinedBits): 0x%" PRIx8 ".\n",
    param.userDefinedBits
  );

  if (param.userDefinedBits && !warningFlags->presenceOfUserDefinedBits) {
    LIBBLU_INFO(LIBBLU_DTS_PREFIX "Presence of an user-data byte.\n");
    warningFlags->presenceOfUserDefinedBits = true;
  }

  LIBBLU_DTS_DEBUG_EXTSS(
    "  Extension Substream Index (nExtSSIndex): %" PRIu8 " (0x%" PRIx8 ").\n",
    param.extSSIdx, param.extSSIdx
  );

  if (
    (param.extSSIdx != 0x0 && param.extSSIdx != 0x2)
    || ((param.extSSIdx == 0x2) ^ isSecondaryStream)
  )
    LIBBLU_DTS_COMPLIANCE_ERROR_RETURN(
      "Unexpected Extension substream index value "
      "(0x%" PRIX8 " parsed, expect 0x0 for primary audio "
  ...
  if (param.staticFieldsPresent) {
    ...
    if (
      checkDcaExtSSHeaderStaticFieldsCompliance(
        param.staticFields, isSecondaryStream, param.extSSIdx, warningFlags
      ) < 0
    )
      return -1;
  }
  ...
  for (nAst = 0; nAst < param.staticFields.nbAudioAssets; nAst++)
    LIBBLU_DTS_DEBUG_EXTSS(
      "   -> Asset %u: %zu bytes.\n",
  ...
  for (nAst = 0; nAst < param.staticFields.nbAudioAssets; nAst++) {
    ...
    if (
      checkDcaAudioAssetDescriptorCompliance(
